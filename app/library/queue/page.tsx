  ...
  return (
    <ScrollArea className='h-full w-full bg-zinc-950 bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(39,39,42,0.8),rgba(255,255,255,0))]'>
      <div className='grid w-full grid-cols-1 gap-2 p-3 md:gap-4 md:p-5'>
        ...
        {current_song ? (
          <>
            <h3 className='truncate text-base font-medium text-zinc-400'>
              Now Playing
            </h3>
            <div className='flex w-full flex-col items-center justify-center'>
              <MyTableRow song={current_song} idx={0} />
            </div>
          </>
        ) : (
        ...
        {queue.playlist && (
          <>
            <h3 className='truncate text-base font-medium text-zinc-400'>
              Next from: {queue.playlist.name}
            </h3>
            <div className='flex w-full flex-col items-center justify-center'>
              {queue.playlist.songs.map((song, idx) => {
                if (idx >= queue.playlist!.idx)
                  return (
                    <MyTableRow
                      song={{ ...song, playlist_id: queue.playlist!.id }}
                      idx={1 + queue.songs.length + idx - queue.playlist!.idx}
                      p_idx={idx}
                      key={song.id}
                    />
                  );
